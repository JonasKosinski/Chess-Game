...
class Board:
    def __init__(self, tiles, turn):
        ...
        self.castable_pieces = np.uint64(0)
        ...
        for piece in tiles.values():
            if piece is not None:
                ...
                elif (piece.type == 'King' or piece.type == 'Rook') and not piece.moved:
                    self.castable_pieces = self.castable_pieces | bb_tiles[i]
    ...
    def apply_move(self, move):
        ...
        # Executes special moves
        if move[2] == 'Pawn Exchange':
            exchange_types = ['Rook', 'Knight', 'Bishop', 'Queen']
            type = exchange_types[move[1]]
            destination = move[0]
            ...
            if tile[0] == 2:
                self.apply_move((tile2index((0, tile[1])), tile2index((3, tile[1])), 'Standard'))
            else:
                self.apply_move((tile2index((7, tile[1])), tile2index((5, tile[1])), 'Standard'))
        ...
        # Moves piece to the destination position
