from bitboard_functions import shift_bb, index2tile, bit_counter, extract_indices, bitscan, tile2index
...
# Applies a rank, file or diagonal mask which contains the the two indexes to a bitboard
def apply_mask_between_2_points(index1, index2, bb):
    tile = index2tile(index1)
    if ranks[tile[1]] & bb_tiles[index2] == bb_tiles[index2]:
        return bb & ranks[tile[1]]

    elif files[tile[0]] & bb_tiles[index2] == bb_tiles[index2]:
        return bb & files[tile[0]]

    elif diag_mask[tile[0] + tile[1]] & bb_tiles[index2] == bb_tiles[index2]:
        return bb & diag_mask[tile[0] + tile[1]]

    elif anti_diag_mask[tile[0] - tile[1]] & bb_tiles[index2] == bb_tiles[index2]:
        return bb & anti_diag_mask[tile[0] - tile[1]]
...
def generate_legal_moves(board):
    ...
    for i in enemy_index:
        ...
        attacked_tiles = attacked_tiles | attack_moves(i, enemy_index[i], enemy_color, occupied_bb)
    ...
    # Being in check limits the number of moves one can make to capturing pieces that put the king in check
    # or blocking sliding pieces which put the king in check
        ...
        if enemy_index[i] in ('Queen', 'Rook', 'Bishop'):
            ...
            push_mask = apply_mask_between_2_points(friendly_king_index, i, push_mask)
            push_mask = push_mask & attack_moves(friendly_king_index, enemy_index[i], enemy_color, occupied_bb)
    ...
    for i in friendly_index:
        ...
        if type == 'Pawn':
            ...
            # Adds pawn exchange moves
            if tile[1] == 7 or tile[1] == 0:
                for j in range(4):
                    if bb_tiles[i] & (push_mask | checkers) == i:
                        moves.append((i, j, 'Pawn Exchange'))
        ...
        # Finds kings moves including castleing
        if type == 'King':
            m = m & ~king_danger_bb

            if num_checkers == 0 and board.castable_pieces & bb_tiles[friendly_king_index] != np.uint64(0):
                rooks = board.castable_pieces & ranks[king_tile[1]] & ~bb_tiles[friendly_king_index]
                for j in extract_indices(rooks):
                    rook_moves = rank_moves(j, occupied_bb)
                    if rook_moves & bb_tiles[friendly_king_index] != np.uint64(0) \
                            and rook_moves & attacked_tiles == np.uint64(0):
                        tile = index2tile(j)

                        if tile[0] == 0:
                            moves.append((friendly_king_index, tile2index((2, tile[1])), 'Castle'))
                        else:
                            moves.append((friendly_king_index, tile2index((6, tile[1])), 'Castle'))

        # Limits pieces moves if the king is in check
        else:
            m = m & (push_mask | checkers)

        m = m & ~friendly_bb

        # Limits moves for pinned pieces
        if ~bb_tiles[i] & pinned_pieces != pinned_pieces:
            m = apply_mask_between_2_points(i, friendly_king_index, m)
